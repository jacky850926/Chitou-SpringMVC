<%@page import="tw.georgia.model.ArticleDao"%>
<%@page import="javax.management.MBeanAttributeInfo"%>
<%@page
	import="java.sql.Connection, java.util.*, javax.sql.*, javax.servlet.*,  javax.naming.*,java.io.*,java.sql.* ,tw.georgia.model.*,tw.georgia.controller.*"%>
	...
	<form action="article.new" method="get">
		...
		<table border="1">
			...
			<tr>
				...
				<td class="centre"><%=bean.getBigClassID()%></td>
				<td class="centre"><%=bean.getArticleClassID()%></td>
				<td><%=bean.getTitle()%></td>
				<td class="centre"><%=bean.getArticleDate()%></td>
				<td><%=bean.getPhoto()%></td>
				...
				<td class="centre">
					...
					<form action="article.renew" method="post" style="">
						...
						<INPUT TYPE="HIDDEN" value=<%=bean.getBigClassID()%> name="bigClassID"> 
						<INPUT TYPE="HIDDEN" value=<%=bean.getArticleClassID()%> name="articleClassID"> 
						<INPUT TYPE="HIDDEN" value=<%=bean.getTitle()%> name="title"> 
						<INPUT TYPE="HIDDEN" value=<%=bean.getArticleDate()%> name="articleDate"> 
